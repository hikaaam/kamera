	...
	if(foto==2):
		...
		while True:
			...
			for (top, right, bottom, left), name in zip(face_locations, face_names):
				...
				if(name != 'unknown'):
					mydb.commit()                                   
					query = "SELECT nim FROM absensi where nim = '{}' and date_format(created_at,'%Y-%m-%{}') = CURDATE()".format(name,str('d'))
					...
					if(len(hasil)<1):
						...
						msg = 'Hi, {} <br>Anda sudah absen pada tanggal {}'.format(name,hari)
						username = 'hidroponikapps@gmail.com'
						password = 'aizrxwyqyotgxnyr'
